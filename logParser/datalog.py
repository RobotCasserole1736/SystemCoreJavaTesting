...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        print("Usage: datalog.py <folder>", file=sys.stderr)
        sys.exit(1)
    ...
    #  sys.argv[1] is a folder
    # Make a list of all files in the folder ending with .wpilog
    logfiles = glob.glob(folder + "/*.wpilog")
    ...
    for logfile in logfiles:
        ...
        with open(logfile, "r") as f:
            ...
            # Make an output folder for all image files
            # in the same folder as the log directory, with the same name as the log file
            output_folder = logfile[:-7] + "_images"
            try:
                os.mkdir(output_folder)
            except FileExistsError:
                print(f"Output folder {output_folder} already exists, using it")
            except OSError as e:
            ...
            for record in reader:
                ...
                if record.isStart():
                    ...
                    # Dump entries which start as /cams and are raw bytes into .jpeg files in output_folder
                    if entry.name.startswith("/cams") and entry.type == "raw":
                        filename = os.path.join(
                            output_folder, f"{entry.name[6:]}_{record.timestamp}.jpeg"
                        )
                        try:
                            mode = "RGB"  # Pillow expects RGB mode
                            numChannels = 3
                            width, height = 640, 480

                            # Convert raw data to a numpy array
                            raw_data = np.frombuffer(record.data, dtype=np.uint8).reshape(height, width, numChannels)

                            # Convert BGR to RGB by swapping channels
                            rgb_data = raw_data[:, :, ::-1]  # Reverse the last axis (channel order)

                            # Create an image from the RGB data
                            image = Image.fromarray(rgb_data, mode)
